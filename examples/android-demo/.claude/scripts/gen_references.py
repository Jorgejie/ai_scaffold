...
REFERENCES_DIR = PROJECT_ROOT / ".qoder" / "references"
...
def discover_modules() -> List[dict]:
    ...
    if sg.exists():
        return _parse_gradle_modules(sg)
    if sg_kts.exists():
        return _parse_gradle_modules(sg_kts)

    # Maven
    pom = PROJECT_ROOT / "pom.xml"
    if pom.exists():
        return _parse_maven_modules(pom)
    ...
    if pkg_json.exists():
        try:
            ...
            if workspaces:
                return _parse_npm_workspaces(workspaces)
        except json.JSONDecodeError:
            pass
    ...
    if cargo.exists():
        return _parse_cargo_workspace(cargo)

    # Go
    go_mod = PROJECT_ROOT / "go.mod"
    if go_mod.exists():
        return [_scan_module_dir(PROJECT_ROOT, "root")]

    # 单模块项目
    return [_scan_module_dir(PROJECT_ROOT, "root")]


def _parse_gradle_modules(settings_path: Path) -> List[dict]:
    ...
    for match in pattern.finditer(content):
        ...
        modules.append(_scan_module_dir(module_path, name, is_aar=is_aar))
    return modules


def _parse_maven_modules(pom_path: Path) -> List[dict]:
    ...
    for m in re.finditer(r"<module>([^<]+)</module>", content):
        ...
        modules.append(_scan_module_dir(module_path, name))
    return modules


def _parse_npm_workspaces(workspaces: Any) -> List[dict]:
...
def _parse_cargo_workspace(cargo_path: Path) -> List[dict]:
    ...
    for m in re.finditer(r'member\s*=\s*\[([^\]]+)\]', content):
        for member in re.findall(r'"([^"]+)"', m.group(1)):
            ...
            if module_path.exists():
                modules.append(_scan_module_dir(module_path, member.replace("/", "_")))
    return modules if modules else [_scan_module_dir(PROJECT_ROOT, "root")]


# === 模块扫描 ===

def _scan_module_dir(module_path: Path, name: str, is_aar: bool = False) -> dict:
    """扫描一个模块目录的结构信息"""
    info = {
        "name": name,
    ...
    if src_roots:
        ...
        for src_root in src_roots:
            ...
            _build_tree(src_root, "", 4, 0, tree_lines)
        info["tree"] = "\n".join(tree_lines)
...
def main():
    ...
    parser.add_argument("--output", type=str, help="输出文件路径（默认 .qoder/references/_scan.json）")
